...
function startNodeRED(config) {
    ...
    //Additional config: Set the httpStaticAuth property here (from Cloudant) if its there
    if (config.httpStaticAuth && !settings.httpStaticAuth) {
        util.log("Additional code to set httpStaticAuth if it is there");
        util.log("" + config.httpStaticAuth.user + " " + config.httpStaticAuth.pass);
        settings.httpStaticAuth = {
            user: config.httpStaticAuth.user
            , pass: config.httpStaticAuth.pass
        };
    }
